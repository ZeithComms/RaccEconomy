		...
		implements IItemGroupBlock, IVisuallyDifferentBlock
{
	...
	public BlockShop()
	{
		super(Block.Properties.of(Material.METAL).harvestTool(ToolType.PICKAXE).harvestLevel(1).strength(2F).requiresCorrectToolForDrops().dynamicShape().noOcclusion());
	}
	...
	{
		Cast.optionally(world.getBlockEntity(pos), TileShop.class)
				.ifPresent(shop ->
				{
					shop.syncNow();
	...
	public void onRemove(BlockState prevState, World world, BlockPos pos, BlockState newState, boolean flag64)
	{
		if(!prevState.is(newState.getBlock()))
		{
			Cast.optionally(world.getBlockEntity(pos), TileShop.class)
					.ifPresent(ts ->
					{
						InventoryHelper.dropContents(world, pos, ts.storage);
	...
	{
		return Cast.optionally(world.getBlockEntity(pos), TileShop.class)
